...
def unravel_spatial_arg(arg_flat, w, l, h):
    assert (
        not isinstance(arg_flat, torch.Tensor) or arg_flat.dtype == torch.int64
    ), "The following computations require int64 datatype to not break."
    x = arg_flat // (l * h)
    y = (arg_flat - x * l * h) // h
...
def spatial_argmax_3d(data):
    ...
    return a tensor of shape BxCx3 of 3D coordinates corresponding to the
    spatial argmax for each batch element, for each channel."""
    assert (
        len(data.shape) == 3
    ), "spatial_argmax_3d expected data of shape B x C x W x L x H"
