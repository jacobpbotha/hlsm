...
def get_task_traj_data_path(data_split: str, task_id: str) -> str:
    ...
    traj_data_path = os.path.join(
        alfred_root, "data", "json_2.1.0", data_split, task_id, "traj_data.json"
    )
...
def get_traj_data_paths(data_split: str) -> List[str]:
    ...
    for task in all_tasks:
        ...
        for trial in trials:
            traj_data_paths.append(
                os.path.join(traj_data_root, task, trial, "traj_data.json")
            )
...
def get_task_dir_path(data_split: str, task_id: str) -> str:
    alfred_root = get_alfred_root_path()
    task_dir_path = os.path.join(
        alfred_root, "data", "json_2.1.0", data_split, task_id.split("/")[0]
    )
