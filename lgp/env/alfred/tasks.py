...
class AlfredTask(Task):
    def __init__(self, data_split: str, task_id: str, repeat_idx: int):
        super().__init__()
        self.traj_data: TrajData = AlfredAnnotations.load_traj_data_for_task(
            data_split, task_id
        )
    ...
    def get_record(self) -> TaskRecord:
        return TaskRecord(
            self.get_data_split(), self.get_task_id(), self.get_repeat_idx()
        )
    ...
    def make_task_type_filter(cls, allowed_types):
        if len(allowed_types) == 0:
            ...
            task_filter: Callable[["AlfredTask"], bool] = (
                lambda m: m.get_task_type() in allowed_types
            )
        return task_filter
    ...
    def make_task_id_filter(cls, allowed_ids):
        if len(allowed_ids) == 0:
            ...
            task_filter: Callable[["AlfredTask"], bool] = (
                lambda m: m.get_task_id() in allowed_ids
            )
        return task_filter

    @classmethod
    def iterate_all_tasks(
        cls,
        data_splits=("train",),
        task_filter: Union[None, Callable[["AlfredTask"], bool]] = None,
    ) -> Iterator[Tuple["AlfredTask", int]]:
        ...
        for data_split in data_splits:
            ...
            for i, task_id in enumerate(all_task_ids):
                ...
                for repeat_idx in range(num_repeats):
                    ...
                    if task_filter(task):
                        print(
                            f"INCLUDE {count:5d} : {task.get_task_id()}: {str(task)}:{repeat_idx}"
                        )
                        count += 1
                        yield task, count
                    else:
                        print(
                            f"EXCLUDE {count:5d} : {task.get_task_id()}: {str(task)}:{repeat_idx}"
                        )
                        count += 1
...
def task_traj_lengths():
    ...
    for task_type in TASK_TYPES:
        filtered_tasks = [
            m[1] for m in filter(lambda m: m[0] == task_type, tasks_by_type)
        ]
    ...
    print(
        tabulate(
            print_data,
            headers=["Task Type", "Mean", "Stddev", "10th %ile", "90th %ile"],
        )
    )
