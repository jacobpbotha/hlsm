...
from lgp.models.alfred.hlsm.alfred_perception_model import \
    AlfredSegmentationAndDepthModel
...
class PoseInfo:
    """Given all the different inputs from AI2Thor event, constructs a pose
    matrix and a position vector to add to the observation."""
    ...
    def is_close(self, pi: "PoseInfo"):
        horizon_close = math.isclose(
            self.cam_horizon_deg, pi.cam_horizon_deg, abs_tol=1e-3, rel_tol=1e-3
        )
        cam_pos_close = [
            math.isclose(a, b) for a, b in zip(self.cam_pos_enu, pi.cam_pos_enu)
        ]
        rot_close = [
            math.isclose(a, b) for a, b in zip(self.rot_3d_enu_deg, pi.rot_3d_enu_deg)
        ]
        all_close = (
            horizon_close
            and cam_pos_close[0]
    ...
    def simulate_successful_action(self, action: AlfredAction):
        ...
        if action.action_type == "RotateLeft":
            ...
            elif math.isclose(self.rot_3d_enu_deg[2] % 360, 0) or math.isclose(
                self.rot_3d_enu_deg[2] % 360, 360
            ):
                self.cam_pos_enu[0] += MOVE_STEP
            else:
                raise ValueError(
                    "Agent doesn't appear to be on a 90-degree grid! This is not supported"
                )
    ...
    def get_agent_pos(self, device="cpu"):
        cam_pos = [-self.cam_pos_enu[0], -self.cam_pos_enu[1], self.cam_pos_enu[2]]
    ...
    def get_pose_mat(self):
        ...
        R_agent = euler.euler2mat(
            rot_3d_enu_rad[0], rot_3d_enu_rad[1], rot_3d_enu_rad[2]
        )
        ...
        T_agent_to_camflat = np.asarray(
            [
                [
                    R_agent_to_camflat[0, 0],
                    R_agent_to_camflat[0, 1],
                    R_agent_to_camflat[0, 2],
                    0,
                ],
                [
                    R_agent_to_camflat[1, 0],
                    R_agent_to_camflat[1, 1],
                    R_agent_to_camflat[1, 2],
                    0,
                ],
                [
                    R_agent_to_camflat[2, 0],
                    R_agent_to_camflat[2, 1],
                    R_agent_to_camflat[2, 2],
                    0,
                ],
                [0, 0, 0, 1],
            ]
        ...
        T_camflat_to_cam = np.asarray(
            [
                [
                    R_camflat_to_cam[0, 0],
                    R_camflat_to_cam[0, 1],
                    R_camflat_to_cam[0, 2],
                    0,
                ],
                [
                    R_camflat_to_cam[1, 0],
                    R_camflat_to_cam[1, 1],
                    R_camflat_to_cam[1, 2],
                    0,
                ],
                [
                    R_camflat_to_cam[2, 0],
                    R_camflat_to_cam[2, 1],
                    R_camflat_to_cam[2, 2],
                    0,
                ],
                [0, 0, 0, 1],
            ]
        ...
        T_world_to_cam = (
            T_camflat_to_cam
            @ T_agent_to_camflat
            @ T_agent_pos_to_agent
            @ T_world_to_agent_pos
        )
...
class InventoryInfo:
    ...
    def simulate_successful_action(self, action, latest_observaton):
        if action.action_type == "PickupObject":
            selecton = (
                action.argument_mask[None, None]
                * latest_observaton.semantic_image.to(action.argument_mask.device)
            ).sum(dim=(2, 3))
...
class StateTracker:
    """Converts raw RGB images and executed actions to AlfredObservation
    instances that eval:
    ...
    def __init__(
        ...
        if self.reference_seg:
            ...
            self.seg_model = AlfredSegmentationAndDepthModel(hparams).to(
                PERCEPTION_DEVICE
            )
            self.seg_model.load_state_dict(
                torch.load(lgp.paths.get_segmentation_model_path())
            )
            self.seg_model.eval()
        if self.reference_depth:
            self.depth_model = None
        else:
            self.depth_model = AlfredSegmentationAndDepthModel(hparams).to(
                PERCEPTION_DEVICE
            )
            self.depth_model.load_state_dict(
                torch.load(lgp.paths.get_depth_model_path())
            )
    ...
    def _make_observation(self) -> AlfredObservation:
        ...
        if event.frame is not None:
            rgb_image = (
                torch.from_numpy(event.frame.copy())
                .permute((2, 0, 1))
                .unsqueeze(0)
                .half()
                / 255
        ...
        if self.reference_depth:
            depth_image = (
                torch.from_numpy(event.depth_frame.copy()).unsqueeze(0).unsqueeze(0)
                / 1000
            )
        else:
            _, pred_depth = self.depth_model.predict(
                rgb_image.float().to(PERCEPTION_DEVICE)
            )
            depth_image = pred_depth.to(
                "cpu"
            )  # TODO: Maybe skip this? We later move it to GPU anyway
        ...
        if self.reference_seg:
            ...
            pred_seg, _ = self.seg_model.predict(
                rgb_image.float().to(PERCEPTION_DEVICE)
            )
        ...
        if self.latest_observation is not None:
            ...
            ), "Didn't log an action, but got two observations in a row?"
            rgb_diff = (
                (rgb_image - self.latest_observation.rgb_image).float().abs().mean()
            )
            if rgb_diff < 1e-4:
                print(
                    f"Action: {self.latest_action}, RGB Diff: {rgb_diff}. Counting as failed."
                )
                action_failed = True
            else:
                pass
        ...
        if not action_failed and self.latest_action is not None:
            ...
            self.inventory_info.simulate_successful_action(
                self.latest_action, self.latest_observation
            )
            if len(oinv.inventory_object_ids) != len(
                self.inventory_info.inventory_object_ids
            ):
        ...
        if self.latest_extra_events:
            extra_frames = [
                torch.from_numpy(e.frame.copy()).permute((2, 0, 1)).unsqueeze(0).half()
                / 255
    ...
    def _extract_reference_semantic_image(cls, event, device="cpu"):
        """The segmentation images that come from AI2Thor have unstable
        color<->object mappings.

        Instead, we can build up a one-hot object image from the
        dictionary of class masks
        """
        num_objects = segdef.get_num_objects()
