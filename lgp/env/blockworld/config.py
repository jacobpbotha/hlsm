...
def _build_state_vector_channel_definition():
    ...
    idx = 0
    channels = []
    channels.append(ChannelDefinition(idx, "agent", Color(AGENT_COLOR)))
    idx += 1
    channels.append(ChannelDefinition(idx, "wall", Color(WALL_COLOR)))
    idx += 1
    channels.append(ChannelDefinition(idx, "walkable", Color(WALKABLE_COLOR)))
    idx += 1
    for i in range(len(COLORS)):
        channels.append(ChannelDefinition(idx, f"item-{COLORS[i]}", Color(COLORS[i])))
        idx += 1
    for i in range(len(COLORS)):
        # Make rooms a lighter shade by blending with white
        room_color_rgb = 0.2 * np.asarray(
            Color(COLORS[i]).get_rgb()
        ) + 0.8 * np.asarray(Color("white").get_rgb())
        room_color = Color(rgb=room_color_rgb)
        channels.append(ChannelDefinition(idx, f"room-{COLORS[i]}", room_color))
        idx += 1
    return channels
...
def agent_to_room_item_coord(item_coord: Tuple[int, int]) -> Tuple[int, int]:
    return (item_coord[0] + int(ROOM_SIZE / 2), item_coord[1] + int(ROOM_SIZE / 2))
