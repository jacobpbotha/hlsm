    ...
    def __eq__(self, other: Union["Door", Direction]):
        if isinstance(other, Direction):
            ...
            return (
                ...
                and (self.source.coord == other.source.coord)
                and (self.target.coord == other.target.coord)
            )

    def __str__(self):
        print(
            f"Door with direction: {self.direction} from {self.source.coord} to {self.target.coord}"
        )
...
class Room:
    def __init__(
        self, coord: Tuple[int, int], color: str, items: List[Item], doors: List[Door]
    ):
    ...
    def make_random(cls, coord: Tuple[int, int]):
        ...
        num_items = min(
            np.random.poisson(lam=config.ITEM_COUNT_POISSON_LAMBDA), (size - 1) ** 2
        )
        ...
        item_colors = np.random.choice(
            config.COLORS, size=num_items, p=item_color_probs
        )
        ...
        item_coords = cell_coords[
            np.random.choice(len(cell_coords), num_items, replace=False).tolist()
        ].tolist()
    ...
    def get_all_item_cell_coords(cls):
        size = config.ROOM_SIZE
        dim_coords = np.hstack(
            [np.arange(0, (size - 1) / 2), np.arange(math.ceil(size / 2), size)]
        ).astype(np.int32)
        cell_coords = np.stack(np.meshgrid(dim_coords, dim_coords), axis=2).reshape(
            (-1, 2)
        )
        return cell_coords
    ...
    def __hash__(self):
        return hash(
            str(self.coord) + str(self.color) + str(self.items) + str(self.doors)
        )
    ...
    def push_item(self, item: Item):
        overlapping_items = [
            ...
            if x.coord[0] == item.coord[0] and x.coord[1] == item.coord[1]
        ]
    ...
    def pop_item(self, coord: Tuple[int, int]) -> Union[Item, None]:
        matching_items = [
            i
            for i, x in enumerate(self.items)
            if x.coord[0] == coord[0] and x.coord[1] == coord[1]
        ]
    ...
    def add_door(self, other: "Room"):
        assert self.is_physically_adjacent(
            other
        ), "Can't connect rooms that are not adjacent"
        if self.coord[1] < other.coord[1]:
            self.doors.append(
                Door(source=self, target=other, direction=Direction.RIGHT)
            )
            other.doors.append(
                Door(source=other, target=self, direction=Direction.LEFT)
            )
        elif self.coord[1] > other.coord[1]:
            self.doors.append(Door(source=self, target=other, direction=Direction.LEFT))
            other.doors.append(
                Door(source=other, target=self, direction=Direction.RIGHT)
            )
            ...
            other.doors.append(
                Door(source=other, target=self, direction=Direction.DOWN)
            )

    def get_center_coordinate(self):
        top_left_coord = tuple(map(lambda m: m * (config.ROOM_SIZE + 2), self.coord))
        center_coord = tuple(
            map(lambda m: m + int((config.ROOM_SIZE + 2) / 2), top_left_coord)
        )
        return center_coord
    ...
    def get_one_hot_tensor(self):
        ...
        room_channels = [
            idx for name, idx in c_name_to_idx.items() if name.startswith("room")
        ]
