...
def _draw_over(tensor_a, tensor_b, mask=None):
    """Draw the contents of tensor_b over the contents of tensor_a as if it
    were a canvas, replacing information that is already there at all pixels
    where tensor_b (or mask if provided) is non-zero.
...
def one_hot_to_image(tensor_repr: torch.tensor) -> torch.tensor:
    """Maps a BxCxDxD-dimensional tensor with C semantic one-hot channels to a
    Bx3xDxD-dimensional RGB representation, using the colors defined for each
    channel in config.py.
    ...
    with torch.no_grad():
        ...
        channel_color_tensor = torch.tensor(
            [[c.color.rgb for c in channel_defs]], device=tensor_repr.device
        )  # B x C x 3
        world_color_tensor = (
            channel_color_tensor[:, :, :, None, None] * tensor_repr[:, :, None, :, :]
        )  # B x C x 3 x H x W
        ...
        image_repr = world_color_tensor[:, room_channels].sum(
            1
        )  # Sum across semantic channels to obtain a color
