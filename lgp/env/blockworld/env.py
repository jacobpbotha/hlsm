...
from lgp.env.blockworld.bwaction import (Action, ActionType, NavigateArgument,
                                         PickupArgument)
...
class BlockworldEnv(Env):
    ...
    def _transition_function(self, state: World, action: Action) -> (World, float):
        ...
        if action.type == ActionType.NAV:
            assert isinstance(
                action.argument, NavigateArgument
            ), "Incompatible action and argument type"
            ...
            if action.argument.direction in room.doors:
                new_room_coord = tuple(
                    [
                        a + b
                        for a, b in zip(
                            room.coord, displacements[action.argument.direction]
                        )
                    ]
                )
            else:
        ...
        if action.type == ActionType.PICKUP:
            assert isinstance(
                action.argument, PickupArgument
            ), "Incompatible action and argument type"
        ...
        if action.type == ActionType.DROP:
            ...
            if len(next_state.inventory) > 0:
                ...
                coord = config.agent_to_room_item_coord(coord)
                item.coord = (
                    coord  # Update item coordinates to where it's about to be placed
                )
    ...
    def reset(self) -> (BwObservation, BwTask):
        ...
        while True:
            ...
            if oracle_agent.act(
                observation
            ).type != ActionType.STOP and not self.task.check_goal_conditions(
                self.world
            ):
                break
