...
class BwObservation(Observation):
    def __init__(
        ...
        super().__init__()
        assert (
            world_obs.shape[0] == obs_mask.shape[0]
        ), "Batch size of mask and observation must be the same."
        assert (
            world_obs.shape[2] == obs_mask.shape[2]
        ), "Height of mask and observation must be the same."
        assert (
            world_obs.shape[3] == obs_mask.shape[3]
        ), "Width of mask and observation must be the same."
    ...
    def represent_as_image(self) -> torch.tensor:
        ...
        inventory_image = torch.zeros(
            batch_size, self.world_obs.shape[1], 5, self.world_obs.shape[3]
        )
        ...
        complete_image = torch.zeros(
            (
                batch_size,
                state_image.shape[1],
                state_image.shape[2] + 1 + 5,
                state_image.shape[3],
            )
        )
