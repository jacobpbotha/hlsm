...
class AlfredResults:
    ...
    def printout(self):
        ...
        for _type, results in self.results.items():
...
def get_metrics(successes, failures):
    ...
    total_path_len_weight = sum([entry["path_len_weight"] for entry in successes]) + sum(
        [entry["path_len_weight"] for entry in failures]
    )
    completed_goal_conditions = sum([entry["completed_goal_conditions"] for entry in successes]) + sum(
        [entry["completed_goal_conditions"] for entry in failures]
    )
    total_goal_conditions = sum([entry["total_goal_conditions"] for entry in successes]) + sum(
        [entry["total_goal_conditions"] for entry in failures]
    )
