...
def get_metrics(successes, failures):
    """compute overall succcess and goal_condition success rates along with
    path-weighted metrics."""
    # stats
    num_successes, num_failures = len(successes), len(failures)
    num_evals = len(successes) + len(failures)
    total_path_len_weight = sum(
        [entry["path_len_weight"] for entry in successes]
    ) + sum([entry["path_len_weight"] for entry in failures])
    completed_goal_conditions = sum(
        [entry["completed_goal_conditions"] for entry in successes]
    ) + sum([entry["completed_goal_conditions"] for entry in failures])
    total_goal_conditions = sum(
        [entry["total_goal_conditions"] for entry in successes]
    ...
    plw_sr = (
        float(
            sum([entry["path_len_weighted_success_spl"] for entry in successes])
    ...
    plw_pc = (
        float(
            sum([entry["path_len_weighted_goal_condition_spl"] for entry in successes])
...
def compute_alfred_metrics(results: AlfredResults, rollout: List[Dict]):
    ...
    s_spl = (1 if goal_satisfied else 0) * min(1.0, path_len_weight / float(t))
    pc_spl = goal_condition_success_rate * min(1.0, path_len_weight / float(t))
    ...
    print(
        "SR: %d/%d = %.3f"
        % (
            results.results["all"]["success"]["num_successes"],
    ...
    print(
        "GC: %d/%d = %.3f"
        % (
            results.results["all"]["goal_condition_success"][
    ...
    print(
        "PLW SR: %.3f" % (results.results["all"]["path_length_weighted_success_rate"])
    )
    print(
        "PLW GC: %.3f"
        % (results.results["all"]["path_length_weighted_goal_condition_success_rate"])
    )
    print("-------------")
    ...
    for task_type in task_types:
        task_successes = [
            s for s in (list(results.successes)) if s["type"] == task_type
        ]
