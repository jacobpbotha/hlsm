...
class ActionProposal(nn.Module, ABC):
    ...
    def _get_state_for_action(self, action: Action) -> "ActionProposal.ModelState":
        """Given that "action" was executed in the environment since the last
        action proposal, compute the next hidden state."""
        ...
    ...
    @abstractmethod
    def forward(
        self, state: StateRepr, task: TaskRepr, prev_action: ActionRepr
    ) -> ActionDistribution:
        ...
    ...
    def mle(self, state: StateRepr, task: TaskRepr, model_state: ModelState):
    ...
    def propose(
        ...
        action_opts: Iterable[Action] = action_distr.sample(
            max_num_actions, with_replacement=with_replacement
        )
