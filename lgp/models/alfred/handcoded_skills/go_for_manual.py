...
from lgp.ops.spatial_ops import ravel_spatial_arg, unravel_spatial_arg
...
class GoForManualSkill(Skill):
    ...
    def _construct_cost_function(self, state_repr: AlfredSpatialStateRepr):
        ...
        def _cost_fn_b(cx, cy, interact_dist, interact_angle):
            ...
            interact_dist = 0.6
            x_match = (
                (interact_dist - T < cx.abs())
                .logical_and(interact_dist + T > cx.abs())
                .logical_and(T > cy.abs())
                .logical_and(-T < cy.abs())
            )
            y_match = (
                (interact_dist - T < cy.abs())
                .logical_and(interact_dist + T > cy.abs())
                .logical_and(T > cx.abs())
                .logical_and(-T < cx.abs())
            )
