...
class HlsmNavigationModel(LearnableModel):
    ...
    """Given a current state s_t, proposes an action distribution that makes
    sense."""
    ...
    def __init__(self, hparams=None):
        ...
        self.act_type_emb = nn.Embedding(
            AlfredSubgoal.get_action_type_space_dim(), self.hidden_dim
        )
        self.act_arg_emb = nn.Embedding(
            AlfredSubgoal.get_action_arg_space_dim() + 1, self.hidden_dim
        )
    ...
    def mle(self, state: AlfredSpatialStateRepr, object_id: torch.tensor):
        return ...
    ...
    def forward(self, batch: Dict):
        ...
        pos_pred_log_distr, yaw_log_distr, pitch_prediction = self.forward_model(
            features_2d, subgoal_arg_features, subgoals
        )
        ...
        assert (
            flat_gt_pos_prob.sum() == b
        ), "Each ground truth distribution needs to be a simplex"
        ...
        yaw_loss = (
            -(gt_yaw_prob * yaw_log_distr).sum(dim=(1, 2, 3)).mean(dim=0)
        )  # Sum across spatial dims, because only one spatial position is actually non-zero per batch

        # Loss for predicting the pitch at the goal pose (position + yaw)
        gt_pitch = gt_pitch.sum(dim=1, keepdim=True)
        has_pitch = gt_pitch != 0
        pitch_loss = (has_pitch * ((pitch_prediction - gt_pitch) ** 2)).sum() / (
            has_pitch.sum()
        )
        ...
        def map_colors_for_viz(pdist):
            ...
            colors = torch.tensor(
                [[1, 0, 0], [0, 0, 1], [0, 0.5, 0.5], [0.5, 0.5, 0]],
                dtype=pdist.dtype,
                device=pdist.device,
            )
            # B x 4 x 3 x H x W
        ...
        if viz and self.iter.item() % 1 == 0:
            ...
            location_distr_np = (
                location_distr[0].permute((1, 2, 0)).detach().cpu().numpy()
            )
            ...
            # Yaw
            td_yaw_gt_np = (
                map_colors_for_viz(gt_pos_yaw_prob)[0]
                .permute((1, 2, 0))
                .detach()
                .cpu()
                .numpy()
            ...
            yaw_distr_pred_np = (
                map_colors_for_viz(yaw_distr_pred)[0]
                .permute((1, 2, 0))
                .detach()
                .cpu()
                .numpy()
            ...
            if PITCH4:
                ...
                gt_pitch_img = torch.cat(
                    [
                        gt_pitch_img.clamp(0, math.pi) * 0.5,
                        gt_pitch_img.clamp(-math.pi, 0) * (-0.5),
                        gt_pitch_img.clamp(0, 0),
                    ],
                    dim=1,
                )
                ...
                pitch_pred_img = torch.cat(
                    [
                        pitch_prediction[:, 0:1],
                        pitch_prediction[:, 1:2],
                        pitch_prediction[:, 2:3],
                        pitch_prediction[:, 3:4],
                    ],
                    dim=2,
                )
                pitch_pred_img = torch.cat(
                    [
                        pitch_pred_img.clamp(0, 2) * 0.5,
                        pitch_pred_img.clamp(-2, 0) * (-0.5),
                        pitch_pred_img.clamp(0, 0),
                    ],
                    dim=1,
                )
                pitch_pred_img = (
                    pitch_pred_img[0].permute((1, 2, 0)).detach().cpu().numpy()
                )
                comb_pitch_pred = (
                    alpha * td_state_np_tiled + (1 - alpha) * pitch_pred_img
                )
                ...
                pitch_pred_img = torch.cat(
                    [
                        pitch_pred_img.clamp(0, math.pi) * 0.5,
                        pitch_pred_img.clamp(-math.pi, 0) * (-0.5),
                        pitch_pred_img.clamp(0, 0),
                    ],
                    dim=1,
                )
...
lgp.model_registry.register_model(
    "alfred_spatial_navigation_model", HlsmNavigationModel
)
