...
from lgp.flags import BIG_TRACE, GLOBAL_VIZ
...
from lgp.models.alfred.projection.voxel_3d_observability import \
    Voxel3DObservability
...
class HlsmObservationFunction(ObservationFunction):
    ...
    def forward(
        ...
        prev_state: Union[AlfredSpatialStateRepr, None],
        goal: Union[AlfredSubgoal, None],
        ...
        if isinstance(depth_image, DepthEstimate):
            ...
            if self.use_mle_depth:
                ...
                proj_depth = depth_image.get_trustworthy_depth(
        ...
        voxel_grid: VoxelGrid = self.image_to_voxels(
            scene_image, proj_depth, extrinsics4f, hfov_deg, mark_agent=True
        )
        voxel_observability_grid, voxel_ray_depths = self.voxel_3d_observability(
            voxel_grid, extrinsics4f, obs_depth, hfov_deg
        )
        ...
        if prev_state is None:
            next_state = AlfredSpatialStateRepr(
                voxel_grid, voxel_observability_grid, inventory_vector, observation
            )
        # Subsequent observation - integrate it
        else:
            next_voxel_grid_data = (
                prev_state.data.data * (1 - voxel_observability_grid.data)
                + voxel_grid.data * voxel_observability_grid.data
            )
            next_voxel_grid_occupancy = (
                prev_state.data.occupancy * (1 - voxel_observability_grid.data)
                + voxel_grid.occupancy * voxel_observability_grid.data
            )
            next_observability_mask = torch.max(
                voxel_observability_grid.data, prev_state.obs_mask.data
            )
            ...
            next_voxel_grid = VoxelGrid(
                next_voxel_grid_data,
                next_voxel_grid_occupancy,
                voxel_grid.voxel_size,
                voxel_grid.origin,
            )
            next_observability_voxels = VoxelGrid(
                next_observability_mask,
                next_observability_mask,
                voxel_grid.voxel_size,
                voxel_grid.origin,
            )
            ...
            if VIZ_OBSERVATION:
                show_image(
                    observation.represent_as_image(
                        semantic=True, rgb=True, depth=True, horizontal=False
                    )[0]
                    .detach()
                    .cpu(),
                    "Observation",
        ...
        if BIG_TRACE:
            ...
            self.trace["state_repr"] = AlfredSpatialStateRepr(
                next_state.data,
                next_state.obs_mask,
                next_state.inventory_vector,
                observation,
            )
            self.trace["observed_repr"] = AlfredSpatialStateRepr(
                voxel_grid, voxel_observability_grid, inventory_vector, None
            )
