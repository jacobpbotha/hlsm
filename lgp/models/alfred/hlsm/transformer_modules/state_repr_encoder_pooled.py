...
class StateReprEncoderPooled(nn.Module):
    def __init__(self, dmodel, nhead=8):
        ...
        self.state_layer = nn.Linear(
            AlfredSpatialStateRepr.get_num_data_channels() * 2, dmodel
        )
    ...
    def _make_pooled_repr(cls, state_reprs):
        ...
        state_pooled_and_inv = torch.cat(
            [state_pooled, state_reprs.inventory_vector], dim=1
        )
