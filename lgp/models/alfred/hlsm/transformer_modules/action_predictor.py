...
class ActionPredictor(nn.Module):
    def __init__(self, dmodel, joint_prob=False):
        ...
        if self.joint_prob:
            self.linear_b = nn.Linear(
                dmodel * 3, self.num_types + self.num_args * self.num_types
            )
        else:
    ...
    def forward(self, state_embeddings, sentence_embeddings, action_hist_embeddings):
        ...
        if sentence_embeddings.shape[0] == 1 and state_embeddings.shape[0] > 1:
            sentence_embeddings = sentence_embeddings.repeat(
                (state_embeddings.shape[0], 1)
            )

        combined_embedding = torch.cat(
            [state_embeddings, sentence_embeddings, action_hist_embeddings], dim=1
        )
