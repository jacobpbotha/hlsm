...
from lgp.models.alfred.hlsm.transformer_modules.positional_encodings import \
    positional_encoding_1d_flat
from lgp.models.alfred.hlsm.transformer_modules.transformer_layer import \
...
class SubgoalHistoryEncoder(nn.Module):
    def __init__(self, dmodel, ablate_no_acthist=False, ablate_no_posemb=False):
        ...
        self.transformer_layer_a = TransformerEncoderLayer(
            d_model=self.dim, n_head=8, dim_ff=self.dim
        )
    ...
    def _build_attn_masks(self, batch_id, device, add_sos_tok=False, incl_self=False):
        ...
        if add_sos_tok:
            attn_mask = torch.cat(
                [torch.zeros([1, bt], device=device, dtype=torch.float32), attn_mask],
                dim=0,
            )
            attn_mask = torch.cat(
                [
                    torch.ones([bt + 1, 1], device=device, dtype=torch.float32),
                    attn_mask,
                ],
                dim=1,
            )
    ...
    def forward(self, action_seq, batch_id):
        ...
        self_attn_masks_a = self._build_attn_masks(
            batch_id, device, add_sos_tok=True, incl_self=True
        )
        # Allow attending to current representation that only depends on previous actions.
        self_attn_masks_b = self._build_attn_masks(
            batch_id, device, add_sos_tok=True, incl_self=True
        )
        ...
        enc_seq_a, aw_a = self.transformer_layer_a(
            start_and_act_emb, attn_mask=self_attn_masks_a, return_attn=True
        )
        enc_seq_b, aw_b = self.transformer_layer_b(
            enc_seq_a, attn_mask=self_attn_masks_b, return_attn=True
        )
        return enc_seq_b
