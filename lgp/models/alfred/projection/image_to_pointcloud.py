...
from lgp.utils.utils import save_gif, standardize_image
...
class ImageToPointcloud(nn.Module):
    ...
    def forward(self, camera_image, depth_image, extrinsics4f, hfov_deg, min_depth=0.7):
        ...
        intrinsics = make_pinhole_camera_matrix(
            height_px=h, width_px=w, hfov_deg=hfov_deg
        )
        ...
        points_3d_wrt_camera = depth_to_3d(
            depth=depth_image, camera_matrix=intrinsics, normalize_points=False
        )
        ...
        homo_points_3d_wrt_world = torch.einsum(
            "bxhw,byx->byhw", homo_points_3d_wrt_camera, inverse_extrinsics
        )
        ...
        if False:
            ...
            img_w = r3d.render_aligned_point_cloud(
                points_3d_wrt_world, camera_image, animate=True
            )
            img_c = r3d.render_aligned_point_cloud(
                points_3d_wrt_camera, camera_image, animate=True
            )
            ...
            imageio.imsave(
                os.path.join(outdir, f"pointcloud_test_scene_{imgid}.png"),
                standardize_image(camera_image[0]),
            )
            imageio.imsave(
                os.path.join(outdir, f"pointcloud_test_depth_{imgid}.png"),
                standardize_image(depth_image[0]),
            )
