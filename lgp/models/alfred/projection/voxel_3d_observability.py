...
from lgp.models.alfred.projection.projection_ops import (
    project_3d_camera_points_to_2d_pixels, project_3d_points)
from lgp.models.alfred.voxel_grid import VoxelGrid


class Voxel3DObservability(nn.Module):
    """Computes which voxels in the voxel grid are visible, by projecting each
    voxel back into the image."""
    ...
    def forward(
        ...
        dtype = dtype = (
            torch.float32 if "cpu" == str(voxel_grid.data.device) else torch.half
        )
        extrinsics4f = extrinsics4f.to(
            device
        ).float()  # Projects from world coordinates to camera
        ...
        voxel_coordinates_cam_pixels = (
            voxel_coordinates_cam_pixels * voxels_in_image_bounds.int()
        ).int()
        voxel_coordinates_cam_pixels = voxel_coordinates_cam_pixels[
            :, 0:2, :, :, :
        ]  # drop the z coordinate

        # Compute coordinates of each voxel into a 1D flattened image
        voxel_coordinates_in_flat_cam_pixels = (
            voxel_coordinates_cam_pixels[:, 1:2, :, :, :] * iw
            + voxel_coordinates_cam_pixels[:, 0:1, :, :, :]
        )
        flat_voxel_coordinates_in_flat_cam_pixels = (
            voxel_coordinates_in_flat_cam_pixels.view([b, -1]).long()
        )
        ...
        voxel_ray_bounce_depths = flat_voxel_ray_bounce_depths.view(
            [b, 1, w, l, h]
        )  # Unflatten
        ...
        voxel_observability_mask = torch.max(
            ...
            (voxel_grid.data.max(1, keepdim=True).values > 0.2).long(),
        )
        ...
        observability_grid = VoxelGrid(
            ...
            voxel_observability_mask.type(
                dtype
            ),  # Occupancy (same as observability - consider observed voxels to be occupied)
