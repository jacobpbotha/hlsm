...
class VoxelsToImage(nn.Module):
    ...
    def forward(
        ...
        image_data_flat = torch.zeros(
            [b, c, ih * iw], dtype=torch.float32, device=device
        )
        voxel_data_flat = voxel_grid.data.view([b, c, -1])
        point_in_voxel_flat_coords = flat_voxel_coordinates_in_flat_cam_pixels.view(
            [b, 1, -1]
        )
