...
def project_3d_points(mat4f, points):
    ...
    points_projected = torch.einsum(
        f"by{sdf},bxy->bx{sdf}", torch.cat([points, homo_ones], dim=1), mat4f
    )[:, 0:3]
...
def project_3d_camera_points_to_2d_pixels(img_h, img_w, hfov_deg, points_3d_camera):
    ...
    intrinsics = make_pinhole_camera_matrix(
        height_px=img_h, width_px=img_w, hfov_deg=hfov_deg
    )
...
def project_3d_world_points_to_2d_pixels(
    extrinsics4f, img_h, img_w, hfov_deg, points_3d_world
):
    """
    Args:
    ...
    points_px_camera, point_px_depths = project_3d_camera_points_to_2d_pixels(
        img_h, img_w, hfov_deg, points_3d_camera
    )
    return points_px_camera, point_px_depths
...
def scatter_add_and_pool(
    ...
    dest_with_counters = torch.cat(
        [dest_tensor, torch.zeros_like(dest_tensor[:, 0:1, :])], dim=1
    )

    # Source tensor - point cloud data. Add a "counter" layer
    src_with_counters = torch.cat(
        [source_data, torch.ones_like(source_data[:, 0:1, :])], dim=1
    )
