...
class PointcloudVoxelgridIntersection(nn.Module):
    ...
    def forward(self, point_coordinates: torch.Tensor, voxelgrid: VoxelGrid):
        """Given a pointcloud arranged in a 2D spatial grid (B x (3 + C) x H x
        W), where the first 3 channels are 3D coordinates, and given a 3D voxel
        grid mask compute a mask of size Bx1xHxW indicating which points in the
        pointcloud intersect with voxels of value > 0.5."""
        ...
        point_in_voxel_coords_f = (
            point_coordinates - voxelgrid.origin[:, :, None, None]
        ...
        point_in_bounds_mask = torch.logical_and(
            point_in_voxel_coords >= min_bounds[None, :, None, None],
            point_in_voxel_coords < max_bounds[None, :, None, None],
        )
        point_in_bounds_mask = point_in_bounds_mask.min(
            dim=1, keepdim=True
        ).values  # And across all coordinates
        num_oob_points = (
            (point_in_bounds_mask.int() == 0).int().sum().detach().cpu().item()
        )
        ...
        point_in_voxel_flat_coords = (
            point_in_voxel_coords[:, 0] * l * h
            + point_in_voxel_coords[:, 1] * h
            + point_in_voxel_coords[:, 2]
        )
        ...
        point_mask_flat = torch.gather(
            voxeldata_flat, dim=2, index=point_in_voxel_flat_coords.view([b, 1, -1])
        )
