...
from lgp.utils.utils import save_frames, standardize_image
    ...
    for sample in rollout:  # [:-1]:
        ...
        for tok in address:
            try:
                ...
                print(
                    f"Couldn't locate key: {tok} in address: {address}."
...
def draw_text(text, width_px):
    ...
    cv2.putText(
        bg, text, (5, 15), cv2.QT_FONT_NORMAL, color=(255, 255, 255), fontScale=0.3
    )
    return bg
...
def draw_on_img(
    text,
    img_or_shape_or_heightpx,
    color=(0, 0, 0),
    ol_color=(255, 255, 255),
    ...
    if isinstance(img_or_shape_or_heightpx, int):
        ...
        img = (
            np.ones((height_px, width_px, 3), dtype=np.uint8)
            * np.asarray(bg_color)[None, None, :]
        )
    elif isinstance(img_or_shape_or_heightpx, tuple):
        img = (
            np.ones(
                [img_or_shape_or_heightpx[0], img_or_shape_or_heightpx[1], 3],
                dtype=np.uint8,
            )
            * np.asarray(bg_color)[None, None, :]
        )
...
def dynamic_voxel_viz(rollout, live=False):
    ...
    subgoals = extract_frames(
        rollout, "agent_trace::hl_agent::action_proposal::subgoal"
    )
    r_images = extract_frames(
        rollout, "agent_trace::skills::PickupObject::gofor::goto::reward_map"
    )
    v_images = extract_frames(
        rollout, "agent_trace::skills::PickupObject::gofor::goto::v_image"
    )
    ...
    for i in range(len(state_reprs)):
        ...
        gx, gy = (
            r_image[0, 0].max(1).values.argmax(0).item(),
            r_image[0, 0].max(0).values.argmax(0).item(),
        )
        ...
        current_base_color = torch.tensor([1.0, 1.0, 1.0], device=device)[
            None, :, None, None, None
        ]
        past_base_color = torch.tensor([1.0, 1.0, 1.0], device=device)[
            None, :, None, None, None
        ]
        all_rgb_voxelgrid.data = all_rgb_voxelgrid.data * 0.5 + past_base_color * 0.5
        current_rgb_voxelgrid.data = (
            current_rgb_voxelgrid.data * 1.0
        )  # 0.8 + current_base_color * 0.0
        ...
        all_rgb_voxelgrid.data = (
            all_rgb_voxelgrid.data * (1 - current_rgb_voxelgrid.occupancy)
            + current_rgb_voxelgrid.data * current_rgb_voxelgrid.occupancy
        )
        ...
        if subgoal is not None:
            ...
            combined_rgb = torch.clamp(
                all_rgb_voxelgrid.data.data + yellow_action, 0, 1
            )
            print(
                f"Mask sum: {action_mask.data.sum().item()}, max: {action_mask.data.max().item()}"
            )
        else:
            combined_rgb = all_rgb_voxelgrid.data.data

        combined_rgb_voxelgrid = VoxelGrid(
            combined_rgb, all_rgb_voxelgrid.occupancy, voxel_size, origin
        )
        ...
        az = -az
        combined_rgb_voxelgrid.data[
            0, :, int(ax.item()), int(ay.item()), : int(az.item())
        ] = torch.tensor([[0], [0], [0]], device=device).repeat((1, int(az.item())))
        combined_rgb_voxelgrid.occupancy[
            0, :, int(ax.item()), int(ay.item()), : int(az.item())
        ] = 1.0
...
def visualize_corl21(rollout, vizdir, rollout_name, start_t=0):
    ...
    media["rgb_img"] = [
        standardize_image(
            obs.represent_as_image(rgb=True, semantic=False, depth=False)[0],
            scale=2.0,
            normalize=False,
            uint8=True,
        )
    ...
    media["semantic_img"] = [
        standardize_image(
            obs.represent_as_image(rgb=False, semantic=True, depth=False)[0],
            scale=2.0,
            normalize=True,
            uint8=True,
        )
    ...
    media["depth_img"] = [
        standardize_image(
            obs.represent_as_image(rgb=False, semantic=False, depth=True)[0] / 5,
            scale=2.0,
            normalize=False,
            uint8=True,
        )
    ...
    media["action_args"] = [
        standardize_image(
            action.represent_as_image(), scale=2, normalize=True, uint8=True
        )
        for action in actions
    ]
    media["ov_actions"] = [
        0.5 * rgb + 0.5 * act * np.asarray([[[1, 0, 0]]])
        for rgb, act in zip(media["rgb_img"], media["action_args"])
    ]
    ...
    # VIN occupancy image:
    occupancy_2d = extract_frames(
        rollout, "agent_trace::skills::PickupObject::gofor::goto::occupancy_2d"
    )
    ...
    r_images = [
        standardize_image(r[0].repeat((3, 1, 1)), scale=5, uint8=True) for r in r_images
    ]
    ...
    hl_actions_text = [
        f"({a.type_str()}, {a.arg_str()})" if a is not None else "   "
        for a in hl_actions_raw
    ]
    # W = media["rgb_img"][0].shape[1]
    hl_actions = [
        draw_on_img(a, (100, 600), color=(0, 0, 0), bg_color=(255, 255, 255))
        for a in hl_actions_text
    ]
    media["hl_actions"] = hl_actions

    # Action mask buildng
    fpv_argument_mask = extract_frames(
        rollout,
        "agent_trace::skills::PickupObject::interact::fpv_argument_mask",
        standardize=True,
    )
    fpv_voxel_argument_mask = extract_frames(
        rollout,
        "agent_trace::skills::PickupObject::interact::fpv_voxel_argument_mask",
        standardize=True,
    )
    fpv_semantic_argument_mask = extract_frames(
        rollout,
        "agent_trace::skills::PickupObject::interact::fpv_semantic_argument_mask",
        standardize=True,
    )
    llc_flow_states = extract_frames(
        rollout, "agent_trace::skills::PickupObject::interact::llc_flow_state"
    )
    action_intermediate_masks = [
        viz.vstack([a, b, c])
        for a, b, c in zip(
            fpv_argument_mask, fpv_voxel_argument_mask, fpv_semantic_argument_mask
        )
    ]
    ...
    extra_rgb_frames = extract_frames(
        rollout, "agent_trace::obs_func::extra_rgb_frames"
    )
    ...
    for i in range(len(media["rgb_img"])):
        ...
        rgb_images = [
            standardize_image(img, scale=2, uint8=True) for img in rgb_images
        ] + [rgb_img]
        ...
        for j, rgb_img in enumerate(rgb_images):
            ...
            subgoal_text = (
                f"({subgoal.type_str()}, {subgoal.arg_str()})"
                if subgoal is not None
                else " "
            )
            ...
            a_mask_fpv_vox = standardize_image(
                fpv_voxel_argument_mask[k], scale=2, uint8=True
            )
            a_mask_fpv_seg = standardize_image(
                fpv_semantic_argument_mask[k], scale=2, uint8=True
            )
            a_mask_fpv_vox = (
                actbg * 0.4
                + a_mask_fpv_vox * np.asarray([1, 0, 0])[None, None, :] * 0.6
            )
            a_mask_fpv_seg = (
                actbg * 0.4
                + a_mask_fpv_seg * np.asarray([1, 0, 0])[None, None, :] * 0.6
            )

            a_mask_fpv_seg = draw_on_img(
                "Subgoal Argument", a_mask_fpv_seg, max_text_h=50, line_num=0
            )
            a_mask_fpv_seg = draw_on_img(
                "   Semantic Class Mask", a_mask_fpv_seg, max_text_h=50, line_num=1
            )
            a_mask_fpv_vox = draw_on_img(
                "Subgoal Argument", a_mask_fpv_vox, max_text_h=50, line_num=0
            )
            a_mask_fpv_vox = draw_on_img(
                "   Projected Voxel Mask", a_mask_fpv_vox, max_text_h=50, line_num=1
            )
