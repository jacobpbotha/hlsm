...
def draw_on_img(
    ...
    if isinstance(img_or_shape_or_heightpx, int):
        ...
        img = (
            np.ones([img_or_shape_or_heightpx[0], img_or_shape_or_heightpx[1], 3], dtype=np.uint8)
    ...
    img = cv2.putText(
        img, text, origin, cv2.FONT_HERSHEY_PLAIN, lineType=cv2.LINE_AA, color=color, fontScale=font_scale, thickness=1
    )
    return img
...
def dynamic_voxel_viz(rollout, live=False):
    ...
    for i in range(len(state_reprs)):
        ...
        torch.tensor([1.0, 1.0, 1.0], device=device)[None, :, None, None, None]
        ...
        az = -az
        combined_rgb_voxelgrid.data[0, :, int(ax.item()), int(ay.item()), : int(az.item())] = torch.tensor(
            [[0], [0], [0]], device=device
        ).repeat((1, int(az.item())))
...
def export_media(media, vizdir, name, fps_settings={}, start_t=0):
    ...
    for media_name, frames in media.items():
        ...
        fps_settings[media_name] if media_name in fps_settings else 1
...
def visualize_corl21(rollout, vizdir, rollout_name, start_t=0):
    ...
    media["rgb_img"] = [
        standardize_image(
            obs.represent_as_image(rgb=True, semantic=False, depth=False)[0], scale=2.0, normalize=False, uint8=True
        )
        for obs in observations
    ]
    media["semantic_img"] = [
        standardize_image(
            obs.represent_as_image(rgb=False, semantic=True, depth=False)[0], scale=2.0, normalize=True, uint8=True
        )
        for obs in observations
    ]
    media["depth_img"] = [
        standardize_image(
            obs.represent_as_image(rgb=False, semantic=False, depth=True)[0] / 5, scale=2.0, normalize=False, uint8=True
        )
        for obs in observations
    ]
    ...
    media["action_args"] = [
        standardize_image(action.represent_as_image(), scale=2, normalize=True, uint8=True) for action in actions
    ]
    media["ov_actions"] = [
        0.5 * rgb + 0.5 * act * np.asarray([[[1, 0, 0]]]) for rgb, act in zip(media["rgb_img"], media["action_args"])
    ]
    ...
    fpv_argument_mask = extract_frames(
        rollout, "agent_trace::skills::PickupObject::interact::fpv_argument_mask", standardize=True
    )
    fpv_voxel_argument_mask = extract_frames(
        rollout, "agent_trace::skills::PickupObject::interact::fpv_voxel_argument_mask", standardize=True
    )
    fpv_semantic_argument_mask = extract_frames(
        rollout, "agent_trace::skills::PickupObject::interact::fpv_semantic_argument_mask", standardize=True
    )
    llc_flow_states = extract_frames(rollout, "agent_trace::skills::PickupObject::interact::llc_flow_state")
    action_intermediate_masks = [
        viz.vstack([a, b, c]) for a, b, c in zip(fpv_argument_mask, fpv_voxel_argument_mask, fpv_semantic_argument_mask)
    ]
    ...
    for i in range(len(media["rgb_img"])):
        ...
        for j, rgb_img in enumerate(rgb_images):
            k = i if j >= len(rgb_images) - 2 else i - 1
            ...
            ccol = draw_on_img(
                "Semantic Voxel Map", ccol, color=(255, 255, 255), ol_color=(0, 0, 0), max_text_h=30, line_num=0
            )
            ccol = draw_on_img(
                "Black Pillar: Agent", ccol, color=(0, 0, 0), ol_color=(0, 0, 0), max_text_h=30, line_num=1
            )
            ccol = draw_on_img(
                "Red Pillar: Navigation Goal", ccol, color=(255, 0, 0), ol_color=(0, 0, 0), max_text_h=30, line_num=2
            )
            ccol = draw_on_img(
            ...
            # Mark action text
            ccol = draw_on_img(
                f"Task: {task_str}", ccol, color=(255, 255, 255), ol_color=(0, 0, 0), max_text_h=52, line_num=-3
            )
            ccol = draw_on_img(
                f"Subgoal: {subgoal_text}", ccol, color=(204, 255, 153), ol_color=(0, 0, 0), max_text_h=50, line_num=-2
            )
            ccol = draw_on_img(
                f"Action: {action_text}", ccol, color=(153, 204, 255), ol_color=(0, 0, 0), max_text_h=50, line_num=-1
            )
            ccol = draw_on_img(
