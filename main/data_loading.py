...
def make_dataloader(
    data_or_paths, dataset_mode, cls, env_type, hparams, batch_size=64, num_workers=0
):
...
def get_incl_task_indices(env_setup):
    ...
    for i, (alfred_task, task_id) in enumerate(
        AlfredTask.iterate_all_tasks(data_splits, task_filter=task_filter)
    ):
...
def filter_rollout_paths(dataset_paths, incl_indices):
    ...
    filtered_paths = [
        ...
        if int(os.path.basename(f).split("_")[1].split(".")[0]) in incl_indices
    ]
    return filtered_paths


def make_disk_dataloaders(
    dataset_mode, env_setup, max_rollouts, env_type, hparams, batch_size, num_workers
):
    ...
    val_loader = make_dataloader(
        val_paths, dataset_mode, TapmDataset, env_type, hparams, batch_size, num_workers
    )
